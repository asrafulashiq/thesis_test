...
for Data_ID=1:12
    clearvars -except Data_ID varpath datapath ERROR
    close all;clc;
    %%
    switch Data_ID
        case 1
    ...
    %%                          LOAD DATA
    clearvars -except datapath varpath data tag window_no Data_ID ERROR start_steep
    load([datapath data]);load([datapath data '_BPMtrace']);clear data;
...
%     sketch10pseudo_full
    sketch10pseudoUNDENOISED_full
    
    
    
    clc
    ERROR(Data_ID)=mean(err)
    save('ERROR','ERROR')
    
    % acceleroscope
end
overall_mean_error=mean(ERROR)
%%
toc
