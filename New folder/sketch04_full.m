...
while go<=stop
    ...
    pause(.001)
    if window<back_sample
        plot(window,BPM1(window),'g.');
    elseif window==back_sample
        plot(window,BPM1(window),'y.');
    else
        thres=10;
        if pder(window)>thres&pder(window)<50
            ...
            if abs(BPM1(window)-HR0)>5
                ...
                plot(window,BPM1(window),'r.');
                plot(window,predicted_value,'b.');
            else
                plot(window,BPM1(window),'g.');
            end
    ...
    set(gca,'fontsize',12);
    xlabel(['mean error: ' num2str(mean(err))],'fontsize',12)
    ylabel('Bits per minute','fontsize',12)
    title(['Window: ' num2str(window)]);legend boxoff
