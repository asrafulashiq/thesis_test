...
lRLS=40;ii=0;avg=0;
pre_flag=0;pre_flag2=0;pre_flag3=0;pre_flag4=0;
%%
mltplr=round(Fs/125);
sig1=PPGm;
%%
...
while go<=stop
    ...
    % 5
    if window>69
        b=BPM1(window-26:window-1);
        if mean(abs(b-medfilt1(b)))<0.2
            if abs(BPM1(window)-HR0)>5.5
                BPM1(window)=predicted_value;
            end
        end
    end
    ...
    % 6
    %
    if window>1&BPM1(window)-BPM1(window-1)>3.9&BPM1(window)-BPM1(window-1)<4.1
    else
        if window<23&window>13&pre_flag4==0
            [HReE]=EEMD2(sig6_seg,HR0,del_cnt,Fs,wname,PPG1,PPG2,cseg,mltplr);
            if HReE-BPM1(window-1)>3&HReE-BPM1(window-1)<12
                BPM1(window)=HReE;
                pre_flag4=1;
            end
        end
    ...
    % 7 more works need to be done
    if tag==7     % crude crude crude crude crude
    elseif tag==5 % crude crude crude crude crude
        %     elseif tag==10 % crude crude crude crude crude
    else
        if window>start_steep&window<9+start_steep
            for a=1:2
                HRe_UNITY_W(a)=sig2HR_wavelet_shrn(sig2_seg_UNITY(a,:),Fs,wname);
            end
            [a,b]=min(abs(HRe_UNITY_W-HR0));
            HEeDW=HRe_UNITY_W(b);
            
            if BPM1(window)-BPM1(window-1)<1.2
                if HEeDW>BPM1(window-1)&HEeDW>BPM1(window)&abs(HEeDW-BPM1(window))<20
                    BPM1(window)=HEeDW;
                elseif tag==10
                    if window<5+start_steep
                        HPR=next_predict(BPM1(start_steep-6:window-1));
                        BPM1(window)=HPR;
                    else
                        BPM1(window)=HR0;
                    end
                    %                     annpHR2=[annpHR2 HEeDW];
                    %                     annpW2=[annpW2 window];
                end
            end
        end
    end
    ...
    plot(annpW2,annpHR2,'r.');%plot(annpW,annpHR,'r.');
    hold off
    ...
    if window==window_no
        legend(frdfke,'location','southeast');
    else
        legend(strstr1,'location','southeast');
    ...
    temp=BPM1;
    load([varpath 'Scrl03_HRest_' num2str(tag)]);
    BPM1pseudo=BPM1;
    BPM1=temp;
   
    HR0=BPM1pseudo(window);
