...
elseif mnmm<=3.4
    ...
    S_rls=maxFindFromThreshold_shrn(sig1_f_seg,0.8,Fs);
    
    S_a=[];
    dominantPeaks=maxFindFromThreshold_shrn(accXf(cseg),0.6,Fs);
    if length(dominantPeaks)>2
        dominantPeaks=dominantPeaks(1:2);
    end
    S_a=[S_a,dominantPeaks];
    
    dominantPeaks=maxFindFromThreshold_shrn(accYf(cseg),0.6,Fs);
    if length(dominantPeaks)>2
        dominantPeaks=dominantPeaks(1:2);
    end
    S_a=[S_a,dominantPeaks];
    
    dominantPeaks=maxFindFromThreshold_shrn(accZf(cseg),0.6,Fs);
    if length(dominantPeaks)>2
        dominantPeaks=dominantPeaks(1:2);
    end
    S_a=[S_a,dominantPeaks];
    ...
    HRtd=frequency_estimate_shrn(s4fes2,s4fes3,a4fesx,a4fesy,a4fesz,HR0,mltplr);
    if length(f_rls_set)==1 && abs(f_rls_set-HR0)<26.5
        del_cnt=max([0;del_cnt-1]);
        
        %
        %         S_org=findSignalPeaks_shrn([sig6(2,cseg);sig6(3,cseg)],HR0,5,Fs);
        %         TS=[f_rls_set(1);S_org(:)];
        %         [zz,xx]=min(abs(HR0-TS));
        %         HRe=TS(xx);
        HRe=f_rls_set(1);
        %
    ...
    elseif HRtd~=-1&&abs(HRtd-HR0)<12
        ...
        if HRe==-1
            S_a0=findSignalPeaks_shrn([sig6(4,cseg);sig6(5,cseg);sig6(6,cseg)],HR0,10,Fs);
            S_org=findSignalPeaks_shrn([sig6(2,cseg);sig6(3,cseg)],HR0,5,Fs);
            if abs(S_org-S_a0)>3
                HRe=S_org;
            else
